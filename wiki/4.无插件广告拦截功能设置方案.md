...
**建议使用 luci-app-adblock(安装 luci-i18n-adblock-fast-zh-cn) 或 luci-app-adblock-fast(安装 luci-i18n-adblock-zh-cn) 实现 dnsmasq 广告拦截功能，加载对应的 dnsmasq 规则（包括本文所提及的规则）后，效果同本方案，且更灵活**
...
```bash
curl -sSL https://testingcf.jsdelivr.net/gh/AzurePath749/Custom_OpenClash_Rules@refs/heads/main/shell/edit_custom_firewall_rules.sh -o edit_custom_firewall_rules.sh && chmod +x edit_custom_firewall_rules.sh && ./edit_custom_firewall_rules.sh
...
### 4.2.1. anti-AD 广告过滤规则 + GitHub520 加速规则  
...
### 4.2.2. 原版 adblockfilters 去广告规则 + GitHub520 加速规则   
...
### 4.2.3. adblockfilters-modified 去广告规则 + GitHub520 加速规则   
